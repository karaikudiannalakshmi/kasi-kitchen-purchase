...
function MonthView({ setView }) {
  ...
  async function doExcel() {
    ...
    exportExcel(`kasi-bills-${month}`, buildExport().sheets);
  }
  async function doPDF() {
    const { exportElementPDF } = await import('../lib/exporters');
    await exportElementPDF(document.getElementById('ps-bills'), `kasi-bills-${month}`);
  }
...
function UnpaidView() {
  ...
  async function downloadBankFile() {
    ...
    XLSX.writeFile(wb, 'Kasi-CIB-payment.xls', { bookType: 'biff8' });
    ...
    <>
      ...
      <div className="page-head" style={{ marginBottom: 12 }}>
        ...
        <div className="row" style={{ alignItems: 'flex-end' }}>
          ...
          <ReportActions filename="kasi-outstanding" printSheetId="ps-unpaid" excelSheets={excelSheets} />
        </div>
      </div>
      ...
      <div className="muted" style={{ fontSize: 12.5, marginBottom: 10 }}>The bank file and bulk "Mark paid" use only your ticked bills below (or, if none are ticked, everything currently shown — narrow with the month filter first if you want a specific batch).</div>
